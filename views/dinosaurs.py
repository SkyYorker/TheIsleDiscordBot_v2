    ...
    def embed(self) -> Embed:
        if self.state == "empty":
            ...
            if self.limited:
                embed.add_field(
                    name="ℹ️ Ограничение",
                    value="Показаны только первые 25 динозавров. Используйте фильтры или освободите лишних для "
                          "отображения остальных.",
                    inline=False
                )
            embed.set_footer(
...
class DinosaurDeleteSelectView(View):
    ...
    def create_select_menu(self) -> Select:
        ...
        placeholder = (
            f"Вы выбрали: {saved_dino_class}"
            if saved_dino_class
            else "Выберите динозавра для освобождения"
        )
    ...
    def embed(self) -> Embed:
        if self.empty:
            embed = discord.Embed(
                ...
                description="У вас нет динозавров для освобождения.\n\nНажмите 'Вернуться', чтобы выйти.",
                color=discord.Color.orange()
            )
            return embed

        embed = discord.Embed(
            title="🦖 Освобождение динозавра из слота",
            description="*Перед освобождением динозавра, убедитесь, что выбрали правильного!*",
            color=discord.Color.red()
        )

        rules = (
            "⚠️ **Внимание:**\n"
            "• После освобождения динозавра восстановить его будет невозможно.\n"
            "• Проверьте, что вы выбрали нужного динозавра.\n"
            "• Это действие нельзя отменить."
        )

        embed.add_field(
            name="Правила освобождения",
            value=rules,
            inline=False
        )
        ...
        if getattr(self, "limited", False):
            embed.add_field(
                name="ℹ️ Ограничение",
                value="Показаны только первые 25 динозавров. Используйте фильтры или освободите лишних для отображения "
                      "остальных.",
                inline=False
            )

        embed.set_footer(
            text="Освобождайте динозавров с осторожностью",
        ...
        if custom_id == "go_back":
            ...
            if self.selected_dino:
                ...
                if not result or (isinstance(result, tuple) and not result[0]):
                    reason = result[1] if isinstance(result, tuple) and len(
                        result) > 1 else "Не удалось освободить сохраненного динозавра."
                    embed = discord.Embed(
                        title="❌ Ошибка освобождения",
                        description=reason,
                        color=discord.Color.red()
                    )
                    await interaction.response.edit_message(
                ...
                await interaction.response.edit_message(
                    embed=None,
                    view=None,
                    content=f"Динозавр {self.selected_dino} успешно освобожден из сохранённых!"
                )
            else:
                await interaction.response.send_message(
                    "Сначала выберите динозавра для освобождения!",
                    ephemeral=True
                )
